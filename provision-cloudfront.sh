...
function provision_distribution {
  ...
  if [ "${SUB_ENVIRONMENT}" != "" ] && aws cloudformation describe-stacks --stack-name "${STACK_PREFIX}-cloudfront-certificate" &> /dev/null; then
    # shellcheck disable=SC1091
  ...
  export AWS_REGION="eu-west-2"
  PARAMETERS_FILE=$TMP_PARAM_FILE TEMPLATE_URL=https://template-storage-templatebucket-1upzyw6v9cs42.s3.eu-west-2.amazonaws.com/cloudfront-distribution/template-v3.1.0.yaml ./provisioner.sh "${AWS_ACCOUNT}" "${STACK_PREFIX}-cloudfront" cloudfront-distribution LATEST
}
