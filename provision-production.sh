...
# shellcheck disable=SC1091
source "./scripts/read_cloudformation_stack_outputs.sh" "account-data-api-pipeline"
...
export AWS_PROFILE=di-authentication-production-ApprovedAdmin
...
function provision_pipeline {
  ...
  # Account data api pipeline
  PARAMETERS_FILE="configuration/$AWS_ACCOUNT/account-data-api-pipeline/parameters.json"
  PARAMETERS=$(jq ". += [
                            {\"ParameterKey\":\"ContainerSignerKmsKeyArn\",\"ParameterValue\":\"${ContainerSignerKmsKeyArn}\"},
                            {\"ParameterKey\":\"SigningProfileArn\",\"ParameterValue\":\"${SigningProfileArn}\"},
                            {\"ParameterKey\":\"SigningProfileVersionArn\",\"ParameterValue\":\"${SigningProfileVersionArn}\"},
                            {\"ParameterKey\":\"ArtifactSourceBucketArn\",\"ParameterValue\":\"${AccountDataApiArtifactSourceBucketArn}\"},
                            {\"ParameterKey\":\"ArtifactSourceBucketEventTriggerRoleArn\",\"ParameterValue\":\"${AccountDataApiArtifactSourceBucketEventTriggerRoleArn}\"}
                        ] | tojson" -r "${PARAMETERS_FILE}")
